    ...
    private void fillDynamic(String frameDef){
        ...
        for (String line : lines) {
            ...
            if (tokens.length == 4) {
                ...
                if (ecu == null) {
                    Logger.d("Ecu does not exist:" + tokens[FRAME_ECU].trim());
                } else {
                    int frameId = Integer.parseInt(tokens[FRAME_ID].trim(), 16);
                    int interval = DashBoardActivity.car == DashBoardActivity.CAR_ZOE_Q210 || DashBoardActivity.car == DashBoardActivity.CAR_ZOE_R240 ? Integer.parseInt(tokens[FRAME_INTERVAL_ZOE].trim(), 10) : Integer.parseInt(tokens[FRAME_INTTERVAL_FLUKAN].trim(), 10);
