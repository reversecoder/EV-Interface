...
package com.reversecoder.canze.devices;
...
import com.reversecoder.canze.actors.Ecu;
import com.reversecoder.canze.actors.Ecus;
import com.reversecoder.canze.actors.Frame;
import com.reversecoder.canze.actors.Message;
...
public class ELM327 extends Device {
    ...
    protected boolean initDevice(int toughness, int retries) {
        ...
        while (retries-- > 0) {
            Logger.d("ELM327: flushWithTimeout");
            flushWithTimeout(500);
            Logger.d("ELM327: initDevice(" + toughness + "), " + retries + " retries left");
            if (initDevice(toughness)) return true;
        }
        DashBoardActivity.toast(DashBoardActivity.TOAST_ELM, "Hard reset failed, restarting Bluetooth ...");
        Logger.d(("ELM327: Hard reset failed, restarting Bluetooth ..."));
        ...
        (new Thread(new Runnable() {
            ...
            public void run() {
                ...
                Logger.d(("ELM327: stopBluetooth (via DashBoardActivity)"));
                DashBoardActivity.getInstance().stopBluetooth(false);
                // restart BT without reloading all settings
                Logger.d(("ELM327: reloadBluetooth (via DashBoardActivity)"));
                DashBoardActivity.getInstance().reloadBluetooth(false);
            }
        })).start();
    ...
    public boolean initDevice(int toughness) {
        ...
        Logger.d(("ELM327: initDevice (" + toughness + ")"));
        ...
        if (toughness == TOUGHNESS_HARD) {
            ...
            Logger.d("ELM327: version = " + response);
        ...
        if (response.trim().equals("")) {
            ...
            DashBoardActivity.toast(DashBoardActivity.TOAST_ELM, lastInitProblem);
            return false;
        }
        ...
        if (!initCommandExpectOk("atfcsh7e4")) {
//        if (!initCommandExpectOk("atfcsh77b")) {
            lastInitProblem = "ATFCSH7e4 command problem";
            deviceIsInitialized = false;
            return deviceIsInitialized;
        }
        ...
        // atfcsm1          Set flowinitCommandExpectOk control mode 1 (ID and data suplied)
...
//        }
        ...
        if (toughness == TOUGHNESS_HARD) {
            switch (elmVersion) {
                case 13:
                    DashBoardActivity.toast(DashBoardActivity.TOAST_ELM, "ELM ready, version 1.3, should work");
                    break;
                case 14:
                    DashBoardActivity.toast(DashBoardActivity.TOAST_ELM, "ELM ready, version 1.4, should work");
                    break;
                case 15:
                    DashBoardActivity.toast(DashBoardActivity.TOAST_ELM, "ELM is now ready");
                    break;
                case 20:
                    ...
                    break;
                case 8015:
                    DashBoardActivity.toast(DashBoardActivity.TOAST_ELM, "ELM ready, version innocar, should work");
                    break;
    ...
    private void killCurrentOperation() {
        ...
        if (!flushWithTimeoutCore(500, '\0')) {
            Logger.d("ELM327: KillCurrentOperation unable to flush after x");
        }
    }
    ...
    private boolean initCommandExpectOk(String command, boolean untilEmpty, boolean addReturn) {
        Logger.d("ELM327: initCommandExpectOk [" + command + "] untilempty [" + untilEmpty + "]");
        String response = "";
        for (int i = 2; i > 0; i--) {
            if (untilEmpty) {
        ...
        /* if (timeoutLogLevel >= 2 || (timeoutLogLevel >= 1 && !command.startsWith("atma") && command.startsWith("at"))) {
            DashBoardActivity.toast("Err " + command + " [" + response.replace("\r", "<cr>").replace(" ", "<sp>") + "]");
        } */
        DashBoardActivity.toast(DashBoardActivity.TOAST_ELM, "Error [" + command + "] [" + response.replace("\r", "<cr>").replace(" ", "<sp>") + "]");
        Logger.d("ELM327: initCommandExpectOk > Response was > " + response);
...
/*        if(waitMillis>0) {
        ...
        while (!stop && !timedOut) {
            ...
            try {
                        ...
                        if (ch == EOM1 || ch == EOM2 || ch == EOM3) {
                            ...
                            if (!untilEmpty) {
                                if (answerLinesCount <= 0) { // the number of lines is in
                                    Logger.d("ELM327: sendAndWaitForAnswer > stop on decimal char [" + data + "]");
                                    stop = true; // so quit
                ...
                if (Calendar.getInstance().getTimeInMillis() > end) {
                    timedOut = true;
                    // DashBoardActivity.toast("Sum Ting Wong on command " + command);
                }
        ...
        if (timedOut) {
            /* if (timeoutLogLevel >= 2 || (timeoutLogLevel >= 1 && (command==null || (!command.startsWith("atma") && command.startsWith("at"))))) {
                DashBoardActivity.toast("Timeout on [" + command + "][" + readBuffer.replace("\r", "<cr>").replace(" ", "<sp>") + "]");
            } */
            DashBoardActivity.toast(DashBoardActivity.TOAST_ELM, "Timeout on [" + command + "] [" + readBuffer.replace("\r", "<cr>").replace(" ", "<sp>") + "]");
            Logger.d("ELM327: sendAndWaitForAnswer > timed out on [" + command + "] [" + readBuffer.replace("\r", "<cr>").replace(" ", "<sp>") + "]");
            return ("");
        }

        //Logger.d("ALC: "+answerLinesCount+" && Stop: "+stop+" && Delta: "+(Calendar.getInstance().getTimeInMillis()-start));
    ...
    public Message requestFreeFrame(Frame frame) {
        ...
        Logger.d("ELM327: requestFreeFrame: atcra" + emlFilter);
        ...
        Logger.d("ELM327: requestFreeFrame > TIMEOUT = " + generalTimeout);

        hexData = sendAndWaitForAnswer("atma", 20);

        Logger.d("ELM327: requestFreeFrame > hexData = [" + hexData + "]");
    ...
    public Message requestIsoTpFrame(Frame frame) {
        ...
        //Logger.d("R: "+request+" - C: "+pre+field.getToId());
