...
package com.reversecoder.canze.database;
...
import com.reversecoder.canze.classes.TimePoint;
...
public class CanzeDataSource implements FieldListener
{
    ...
    public void insert(Field field)
    {
        if(!Double.isNaN(field.getValue())) {
            //DashBoardActivity.debug("CanzeDataSource: inserting "+field.getValue()+" for "+field.getSID());
    ...
    public double getLast(String sid)
    {
        ...
        //DashBoardActivity.debug("CanzeDataSource: getting last for "+sid);
        c.moveToFirst();
        if (!c.isAfterLast()) {
            data = c.getDouble(c.getColumnIndex("value"));
            //DashBoardActivity.debug("CanzeDataSource: got value "+data);
        }
