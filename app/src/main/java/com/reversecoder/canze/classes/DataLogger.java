...
//  public static final String SID_EVC_RealSpeed                        = "7ec.622003.24"; //  (EVC)
    ...
    SimpleDateFormat sdf = new SimpleDateFormat(DashBoardActivity.getStringSingle(R.string.format_YMDHMS), Locale.getDefault());
    ...
    private Runnable runnable = new Runnable() {
        ...
        public void run() {
            ...
            String DateString = getDateString( 0 , DashBoardActivity.getStringSingle(R.string.format_YMDHMS));
    ...
    private void addListener(String sid, int intervalMs) {
        Field field;
        field = DashBoardActivity.fields.getBySID(sid);
        if (field != null) {
            field.addListener(this);
            // DashBoardActivity.device.addActivityField(field, intervalMs);
            DashBoardActivity.device.addApplicationField(field, intervalMs);
            subscribedFields.add(field);
        }
        else
        {
            DashBoardActivity.toast("sid " + sid + " does not exist in class Fields");
        }
    }
